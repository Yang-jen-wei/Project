	...
	public ArrayList<String> FindRelativeWords()throws IOException
	{
		...
		String Inputword = searchKeyword.toUpperCase();
	        try {
	            ...
	            Elements elements = doc.body().select("b");     //select all elements in the <b>
	            for (Element ele : elements) {           		//get all elements
	            	if (!ele.ownText().equals("")){
	            		String temp = ele.ownText();
	            		if (!temp.toUpperCase().equals(Inputword)  && findLCS(temp.toUpperCase() , Inputword ) < Inputword.length()-1)              //if the text in a element is not ""
	            			RelativeWords.add(temp);			//then add it to RelativeWords
